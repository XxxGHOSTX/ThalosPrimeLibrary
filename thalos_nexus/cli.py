...
def cmd_evolve(args: argparse.Namespace) -> int:
    ...
    On Windows, gate subprocesses are executed through :class:`IsolationAdapter`
    which enforces Job Object memory limits, an ephemeral workspace, and
    Windows Firewall network-egress blocking.  On non-Windows platforms the
    command exits immediately with a non-zero code.
    ...
    run_dir = Path(args.run_dir).resolve() if args.run_dir else Path("nexus_runs") / run_id
    ...
    if sys.platform != "win32":
        msg = (
            f"evolve requires Windows (sys.platform={sys.platform!r}). "
            "IsolationAdapter is not available on this platform."
        )
        logger.error(msg)
        writer.append(
            "evolve_error",
            {"reason": "non_windows_platform", "platform": sys.platform},
        )
        log_sha = _compute_file_sha256(log_path)
        ...
        kp = load_or_generate_keypair(key_dir)
        store.write_json("repro_manifest.json", sign_manifest(manifest, kp))
        print(f"evolve FAIL: {msg}", file=sys.stderr)
        return 1

    from thalos_nexus.lysosome.windows_adapter import IsolationAdapter, IsolationConfig

    workspace_base = run_dir / "workspace"
    workspace_base.mkdir(exist_ok=True)
    isolation_cfg = IsolationConfig(
        workspace_base=workspace_base,
        timeout_seconds=300.0,
        enable_network=False,
    )
    adapter = IsolationAdapter(isolation_cfg)
    ...
    gate_path_full, gate_sha_full = store.write_json("gate_results.json", gate_data_full)
    log_sha_final = _compute_file_sha256(log_path)
    ...
    manifest_full: dict[str, Any] = {
        ...
        "artifacts": {
            "gate_results": store.make_artifact_ref(
                "gate_results", gate_path_full, gate_sha_full
            ),
            "event_log": store.make_artifact_ref("event_log", log_path, log_sha_final),
        },
    }

    kp = load_or_generate_keypair(key_dir)
    signed = sign_manifest(manifest_full, kp)
    store.write_json("repro_manifest.json", signed)
