...
make audit-search # Generate advanced search audit report (JSON + Markdown)
make audit-search-remote # Include consented hybrid remote scenario in audit
>>>>>>> 14e1313 (chore: commit all current workspace changes)
```

### Advanced Optimization Audit (Real Outputs)

Run the repository-level advanced search audit:

```bash
make audit-search
# Optional: include consent-enabled hybrid remote federation scenario
make audit-search-remote
```
...
The audit compares baseline, enhanced, and adaptive search scenarios using
real in-repo execution and reports:
- latency
- NDCG quality
- diversity and novelty
- concrete "still needed" optimization gaps
