...
class SymbolicConstraintEngine(BaseLifecycleComponent):
    ...
    def _build_z3_vars(
        ...
        for var in variables:
            ...
            if var.lower_bound is not None and var.sort != VariableSort.BOOL:
                numeric_var = cast("z3.ArithRef", z3_var)
                bounds.append(numeric_var >= var.lower_bound)
            if var.upper_bound is not None and var.sort != VariableSort.BOOL:
                numeric_var = cast("z3.ArithRef", z3_var)
                bounds.append(numeric_var <= var.upper_bound)
    ...
    def _parse_constraint(
        ...
        try:
            ...
            if isinstance(result, z3.BoolRef):
                return result
            logger.warning("Constraint did not evaluate to BoolRef: %r", constraint_str)
            return None

    def _parse_objective_expression(
        self,
        expression_str: str,
        z3_vars: dict[str, z3.ExprRef],
    ) -> z3.ArithRef | None:
        """Parse an optimization expression into a numeric Z3 expression."""
        if not _CONSTRAINT_SAFE_PATTERN.match(expression_str):
            logger.warning("Objective contains unsafe characters: %r", expression_str)
            return None
        try:
            builtins_dict: dict[str, object] = {"__builtins__": {}}
            result = eval(  # noqa: S307  # nosec B307 - sandboxed with empty builtins and validated
            ...
            logger.warning("Cannot parse objective %r: %s", expression_str, exc)
            return None
        else:
            if isinstance(result, z3.ArithRef):
                return result
            logger.warning("Objective did not evaluate to ArithRef: %r", expression_str)
            return None
    ...
    def optimize(
        ...
        obj_expr = self._parse_objective_expression(objective.expression, z3_vars)
        if obj_expr is None:
            return SymbolicSolution(
                satisfiable=False,
                model={},
