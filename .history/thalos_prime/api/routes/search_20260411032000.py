...
REMOTE_RETRY_AFTER_S = 300
REMOTE_DISABLED_UNTIL_EPOCH = 0.0
...
def _adapt_profile(profile: IntentProfile, query_term_count: int, enabled: bool) -> IntentProfile:
    if not enabled:
        return profile

    if profile["label"] == "definition":
        coherence_weight = 0.62 if query_term_count <= 3 else 0.58
        lexical_weight = 0.18
        semantic_weight = 0.17
        ensemble_weight = 0.03
    else:
        coherence_weight = 0.44
        lexical_weight = 0.16
...
def _fetch_remote_pages(query: str, max_results: int) -> tuple[list[dict[str, str]], dict[str, Any]]:
    ...
    global REMOTE_DISABLED_UNTIL_EPOCH
    ...
    diagnostics: dict[str, Any] = {
        ...
        "fallback_used": None,
        "remote_retry_after_s": 0,
        "error": None,
    }

    now = time.time()
    if now < REMOTE_DISABLED_UNTIL_EPOCH:
        diagnostics["error"] = "remote_temporarily_suspended"
        diagnostics["fallback_used"] = "local_indexed_corpus"
        diagnostics["remote_retry_after_s"] = int(REMOTE_DISABLED_UNTIL_EPOCH - now)
    ...
    try:
        ...
        diagnostics["fallback_used"] = "local_indexed_corpus"
        REMOTE_DISABLED_UNTIL_EPOCH = time.time() + REMOTE_RETRY_AFTER_S
        return _fetch_local_indexed_pages(query, max_results), diagnostics
...
def _effective_diversity_lambda(
    ...
    if profile["label"] == "definition":
        target = 0.93 if query_term_count <= 2 else 0.88
    else:
        target = 0.48 if query_term_count >= 4 else 0.56

    blended = (request.diversity_lambda * 0.25) + (target * 0.75)
...
async def search(request: SearchRequest) -> SearchResponse:
    ...
    try:
        ...
        intent_profile = _adapt_profile(
            intent_profile,
            query_term_count=len(query_term_set),
            enabled=request.enable_adaptive_optimization,
        )
