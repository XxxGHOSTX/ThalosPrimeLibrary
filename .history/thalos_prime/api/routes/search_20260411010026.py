...
def _intent_profile(query: str) -> dict[str, float]:
...
def _fetch_remote_pages(query: str, max_results: int) -> list[dict[str, str]]:
    ...
    request = urllib.request.Request(search_url, headers={"User-Agent": "ThalosPrimeSearch/1.0"})
    ...
    for link in links[: max_results * 2]:
        page_request = urllib.request.Request(link, headers={"User-Agent": "ThalosPrimeSearch/1.0"})
        try:
