...
class LifecycleValidator(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> None:
        ...
        self.class_bases[node.name] = [
            self._base_name(base) for base in node.bases if self._base_name(base) is not None
        ]
...
def is_subsystem_class(
    ...
    # Classes explicitly bound to lifecycle contracts are always validated.
    lifecycle_bases = {"BaseLifecycleComponent", "LifecycleProtocol"}
    inherits_lifecycle_contract = any(base in lifecycle_bases for base in base_names)

    # Treat classes with 3+ lifecycle methods as subsystem implementations.
    implemented_lifecycle_methods = sum(value is not None for value in methods.values())
    has_lifecycle_method_cluster = implemented_lifecycle_methods >= 3

    # Check if name suggests it's a subsystem
    has_subsystem_name = any(keyword in name_lower for keyword in lifecycle_keywords)

    return inherits_lifecycle_contract or (has_lifecycle_method_cluster and has_subsystem_name)
...
def validate_file(file_path: Path) -> Tuple[List[str], int]:
    ...
    for class_name, methods in validator.classes.items():
        base_names = validator.class_bases.get(class_name, [])
        if not is_subsystem_class(class_name, methods, base_names):
            continue
