...
**Components**:
- HTTP endpoint (`POST /api/v1/chat/`)
...
#### GET /health
- **Purpose**: Liveness/health probe for runtime infrastructure
- **Response**:
  ```json
  {
    "status": "healthy",
...
### Executed Validation Pipelines (Real Outputs)

The following commands were executed in this repository and produced the
recorded outputs below.

1. Type-check pipeline (`make typecheck`)
...
2. Deterministic search benchmark (`python tools/benchmark_search.py --max-results 5 --mode local`)

```text
query   ndcg_baseline   ndcg_enhanced   diversity_baseline      diversity_enhanced
define deterministic coherence  1.0000  1.0000  1.0000  1.0000
explore relationships between graph reasoning and language      1.0000  0.9999  1.0000  1.0000
what is symbolic constraint optimization        1.0000  0.9996  1.0000  1.0000
compare narrative structure and lexical density 1.0000  0.9999  1.0000  1.0000
how do retrieval and generation interact        1.0000  1.0000  1.0000  1.0000

summary
avg_ndcg_baseline=1.0000
avg_ndcg_enhanced=0.9999
avg_diversity_baseline=1.0000
avg_diversity_enhanced=1.0000
```

3. Full repo quality gate (`make check`)

- Pipeline executes type checks, linting, tests, and validators.
- Current status: passing (latest execution in this workspace).
