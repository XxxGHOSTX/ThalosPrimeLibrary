...
class SemanticCLI:
    ...
    def __init__(self, orchestrator: SemanticOrchestrator) -> None:
        """Initialize the CLI with *orchestrator* and derive a deterministic session ID."""
        self.orchestrator = orchestrator
        self.session_id = self._derive_session_id()
        self.verbose = False

    def run(self) -> None:
        """Enter the interactive input loop until EOF, interrupt, or /quit."""
        self._print_banner()
        while True:
            try:
    ...
    def _handle_command(self, command: str) -> bool:
        """Handle a slash command; return False to exit the CLI loop."""
