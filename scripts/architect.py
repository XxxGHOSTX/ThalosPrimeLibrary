...
class ThalosArchitect:
    ...
    REQUIRED_FILES = {
        ...
        "Dockerfile": "Canonical container build definition.",
        "docker-compose.yml": "Canonical compose deployment definition.",
    ...
    def deploy_standalone(self) -> bool:
        """Launch Thalos Prime using docker compose (canonical deploy path)."""
        mode = self._read_env("TPAA_DEPLOY_MODE", "auto").lower()
        health_port = int(self._read_env("TPAA_HEALTH_PORT", "8000"))

        if mode in {"auto", "docker"} and (self.root / "docker-compose.yml").exists():
            try:
                subprocess.check_call(
                    ["docker", "compose", "up", "-d", "--build"],
                    cwd=self.root,
                )
                self.audit["deployment"] = {"mode": "docker-compose", "port": health_port}
                self._record_step("deploy", True, f"docker compose deployment started on port {health_port}")
                return True
            except Exception as exc:
                self.logger.warning("docker compose deployment failed, falling back to local mode: %s", exc)

        target = next(
            (ep for ep in ["run_thalos.py"] if (self.root / ep).exists()),
            None,
        )
        if target:
            subprocess.Popen(
